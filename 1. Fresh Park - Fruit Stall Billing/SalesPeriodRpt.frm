...
If rs.State = 1 Then rs.Close
rs.Open "Select sum(itemamt) from tbl_sales where salesdate between #" & DTPicker1.Value & "# and #" & DTPicker2.Value & "#", db, adOpenDynamic, adLockOptimistic
bamt = rs.Fields(0)
ibamt = 13 - Len(Format(bamt, "0.00"))

If rs.State = 1 Then rs.Close
stmt = "select distinct billno,salesdate,billamt from tbl_sales where salesdate between #" & DTPicker1.Value & "# and #" & DTPicker2.Value & "#"
rs.Open stmt, db, adOpenDynamic, adLockOptimistic
If Not rs.EOF Then
    '----------Notepad print------------------
    Open App.Path & "\report1.txt" For Output As #1
    ...
    Print #1, Space(38) & "------------"
    Close #1
    retval = Shell("notepad.exe report1.txt", vbMaximizedFocus)
End If
