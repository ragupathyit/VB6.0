...
If Not rs.EOF Then
    ...
    Print #1, ""
    Print #1, Space(1) & "B. No: " & rs.Fields("billno") & Space(18) & Format(Date, "DD/MM/YY") & " (" & Format(Time, "HH:MM") & ")"
    Print #1, "---------------------------------------------"
    ...
    Print #1, ""
    Close #1
    'retval = Shell("notepad.exe bill.txt", vbMaximizedFocus)
...
If KeyCode = 117 Then 'F6 Key for Delete the row
    If Not MSGrid.Rows = 1 Then
        lblbillamt.Caption = Format(Val(lblbillamt.Caption) - Val(MSGrid.TextMatrix(MSGrid.Row, 4)), "0.00")
        MSGrid.Row = MSGrid.Row
        MSGrid.Col = 1
