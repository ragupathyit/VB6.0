...
Begin VB.Form CustomerFrm 
   ...
   Begin MSFlexGridLib.MSFlexGrid MSGrid 
      ...
      AllowUserResizing=   2
      Appearance      =   0
      FormatString    =   "Cust ID|Customer Name                                             |Phone/Cell No     "
...
db.Execute "delete from tbl_custmaster where cid=" & lblhide.Caption
...
If rs.State = 1 Then rs.Close
rs.Open "select * from tbl_custmaster where cid=" & lblhide.Caption, db, adOpenDynamic, adLockOptimistic
If Not rs.EOF Then
    rs.Fields("cid") = lblhide.Caption
    rs.Fields("customername") = UCase(Text1.Text)
    rs.Fields("customeraddress") = UCase(Text2.Text)
...
ElseIf Text2.Text = "" Then
    MsgBox "Enter the Customer Address Properly...", vbInformation, "Sumathi Stores"
    Text2.SetFocus
...
'-------------------Validation Ends Here-------------------------------
    ...
    If rs.State = 1 Then rs.Close
    rs.Open "select * from tbl_custmaster order by cid", db, adOpenDynamic, adLockOptimistic
    If Not rs.EOF Then
        rs.MoveLast
        s = rs.Fields("cid") + 1
    Else
    ...
    If rs.State = 1 Then rs.Close
    rs.Open "select * from tbl_custmaster where customername='" & UCase(Text1.Text) & "'", db, adOpenDynamic, adLockOptimistic
    If rs.EOF Then
        rs.AddNew
        rs.Fields("cid") = CInt(s)
        rs.Fields("customername") = UCase(Text1.Text)
        rs.Fields("customeraddress") = UCase(Text2.Text)
        rs.Fields("phone") = UCase(Text3.Text)
        rs.Fields("email") = UCase(Text4.Text)
        rs.Update
        MsgBox "Customer Details Saved Successfully", vbInformation, "Sumathi Stores"
        rs.Close
    Else
        MsgBox "Enter The Another Name for Customer", vbInformation, "Sumathi Stores"
    End If
    Call cmdclear_Click
End If
End Sub
...
If rs.State = 1 Then rs.Close
rs.Open "select * from tbl_custmaster where cid=" & MSGrid.TextMatrix(MSGrid.Row, 0), db, adOpenDynamic, adLockOptimistic
If Not rs.EOF Then
    Text1.Text = rs.Fields("customername")
    If Not rs.Fields("customeraddress") = "" Then
        Text2.Text = rs.Fields("customeraddress")
    End If
...
stmt = "select * from tbl_custmaster order by cid"
If rs.State = 1 Then rs.Close
...
If Not rs.EOF Then
    rs.MoveFirst
    While Not rs.EOF
        MSGrid.AddItem rs.Fields("cid") & vbTab & rs.Fields("customername") & vbTab & rs.Fields("phone")
        rs.MoveNext
    Wend
End If
rs.Close
