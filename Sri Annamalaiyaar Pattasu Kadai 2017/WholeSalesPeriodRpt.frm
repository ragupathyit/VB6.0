...
Begin VB.Form WholeSalesPeriodRpt 
   BackColor       =   &H00C0E0FF&
   BorderStyle     =   0  'None
   Caption         =   "Whole Sales Period Wise Report"
   ClientHeight    =   3765
   ClientLeft      =   0
   ClientTop       =   -15
   ClientWidth     =   8400
   ...
   End
End
Attribute VB_Name = "WholeSalesPeriodRpt"
...
rs.Open "Select * from tbl_sales where salestype='Whole' and salesdate between #" & DTPicker1.Value & "# and #" & DTPicker2.Value & "#", db, adOpenDynamic, adLockOptimistic
If rs.EOF Then
...
If rs.State = 1 Then rs.Close
rs.Open "Select sum(itemamt) from tbl_sales where salestype='Whole' and salesdate between #" & DTPicker1.Value & "# and #" & DTPicker2.Value & "#", db, adOpenDynamic, adLockOptimistic
gamt = rs.Fields(0)

If rs.State = 1 Then rs.Close
rs.Open "Select distinct billno,discount from tbl_sales where salestype='Whole' and salesdate between #" & DTPicker1.Value & "# and #" & DTPicker2.Value & "#", db, adOpenDynamic, adLockOptimistic
dis = 0
While Not rs.EOF
...
stmt = "select itemcode,itemname,itemtype,itemrate,quantity,itemamt,gridtotamt,discount,totamt from tbl_sales where salestype='Whole' and salesdate between #" & DTPicker1.Value & "# and #" & DTPicker2.Value & "#"
If rs.State = 1 Then rs.Close
