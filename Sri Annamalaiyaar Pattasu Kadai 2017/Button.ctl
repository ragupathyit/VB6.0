...
Private Const cbVersion As String = "1.0.0"
...
Public Enum ButtonTypes
  [Windows 16-bit] = 1    ' Old-fashioned Win16 button
  [Windows 32-bit] = 2    ' Classic windows button
  [Windows XP] = 3        ' New brand XP button totally owner-drawn
  [Mac] = 4               ' Mac button
  ...
  [Simple Flat] = 7       ' Standard flat button seen on toolbars
  [Flat Highlight] = 8    ' Flat button but this one has no border until the mouse is over it
  [Office XP] = 9         ' New Office XP button
  [Transparent] = 11      '
  [3D Hover] = 12         ' This came from "Noteworthy Composer" toolbal
  [Oval Flat] = 13        ' A simple Oval Button
  [KDE 2] = 14            ' Standard KDE2 button!
End Enum
...
Private He As Long                        ' Height of the button
Private Wi As Long                        ' Width of the button

Private BackC As Long                     ' BackColor
Private BackO As Long                     ' BackColor when mouse is over
Private ForeC As Long                     ' ForeColor
Private ForeO As Long                     ' ForeColor when mouse is over
...
Private Sub UserControl_Click()
 If LastButton = 1 And isEnabled Then
   ...
   Call Redraw(0, True) ' Besure that the normal status is drawn
   UserControl.Refresh
   RaiseEvent Click
 End If
End Sub
...
Private Sub UserControl_Initialize()
 '
 ' This makes the control to be slow, remark this line if the "not redrawing" problem
 ' is not important for you: ie, you intercept the Load_Event (with breakpoint or messageBox)
 ' and the button does not repaint...
 '
 isShown = True
End Sub
...
' would alter the look of these buttons.
'
...
Public Property Set PictureOver(ByVal newPic As StdPicture)
 ...
 If isOver Then Call Redraw(lastStat, True)  ' Only redraw we need to see this picture immediately
...
Private Sub DrawRectangle(ByVal X As Long, ByVal Y As Long, ByVal Width As Long, ByVal Height As Long, ByVal Color As Long, Optional OnlyBorder As Boolean = False)
 '
 ' This function draws rectangles and frames
...
Private Sub SetColors()
 ...
 If MyColorType = Custom Then
   ...
   cHighLight = ShiftColor(cFace, &H2F)    ' It should be 3F but it looks too light
...
Private Sub SetAccessKeys()
 ...
 '
 ' We only do a second pass because no one writes text like "Me & them & everyone"
...
Private Sub DrawCaption(ByVal State As Byte)
 ...
 ' it is placed here for easier readability
 ...
 With UserControl
   ...
   ' Now draw the text that is common in all the captions
